...
struct Sigmoid {
    static Vector Apply(const Vector& x) {
        return x.unaryExpr([](DataType x) { return 1.0 / (1.0 + std::exp(-x)); });
    }

    static Matrix GetDifferential(const Vector& x) {
        Vector applied_sigmoid = Apply(x);
        Vector differential = applied_sigmoid.unaryExpr([](DataType x) { return (1.0 - x) * x; });
...
struct Tanh {
    static Vector Apply(const Vector& x) {
        return x.unaryExpr([](DataType x) { return std::tanh(x); });
    }

    static Matrix GetDifferential(const Vector& x) {
        Vector applied_tanh = Apply(x);
        Vector differential = applied_tanh.unaryExpr([](DataType x) { return 1.0 - x * x; });
        return differential.asDiagonal();
    }
};
...
struct ReLU {
    static Vector Apply(const Vector& x) {
        return x.unaryExpr([](DataType x) { return std::max(0.0, x); });
    }

    static Matrix GetDifferential(const Vector& x) {
        Vector differential = x.unaryExpr([](DataType x) { return x > 0.0 ? 1.0 : 0.0; });
        return differential.asDiagonal();
    }
};
...
struct Softmax {
    static Vector Apply(const Vector& x) {
        Vector tmp = x.unaryExpr([](DataType x) { return std::exp(x); });
...
Index ActivationFunc::GetFuncId() const {
    return func_id_;
}
