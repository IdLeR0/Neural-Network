...
Vector CrossEntropy::GetGradient(const Vector &y_out, const Vector &y_expected) {
...
Matrix GetGradient(const Matrix &y_out, const Matrix &y_expected) {
    ...
    for (int i = 0; i < y_out.cols(); ++i) {
        VectorT grad_of_one_vector = res.row(i) = GetGradient(y_out.col(i), y_expected.col(i));
    }
    return res;
}
